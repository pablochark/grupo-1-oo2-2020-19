...
public class LocalController {
	...
	public ModelAndView get(@PathVariable("id") long idLocal) {
		ModelAndView mAV = new ModelAndView(ViewRouteHelpers.LOCAL_UPDATE);
		mAV.addObject("local", localService.findById(idLocal));
		return mAV;
	}
	...
	public ModelAndView sacardistancia(LocalesModel locales) {
		...
		double distancia=	distanciaCoord(localService.findById(locales.getLocal1().getIdLocal()).getLatitud(), localService.findById(locales.getLocal1().getIdLocal()).getLongitud(), localService.findById(locales.getLocal2().getIdLocal()).getLatitud(), localService.findById(locales.getLocal2().getIdLocal()).getLongitud());
